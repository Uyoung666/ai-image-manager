...
for (const f of files) {
  ...
  try {
    ...
    if (!meta.width || !meta.height || !meta.format) {
      failures.push({ file: f, size: stat.size, reason: `Incomplete metadata: ${JSON.stringify(meta)}` });
    }
  } catch (err) {
    failures.push({ file: f, size: stat.size, reason: err.message?.slice(0, 100) });
  }
}
...
console.log(`Files 1KB-100KB: ${sizes.filter((s) => s >= 1024 && s < 102400).length}`);
console.log(`Files 100KB-1MB: ${sizes.filter((s) => s >= 102400 && s < 1048576).length}`);
console.log(`Files > 1MB: ${sizes.filter((s) => s >= 1048576).length}`);
