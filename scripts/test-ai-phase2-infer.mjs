...
if (!(MANIFEST_PATH && fs.existsSync(MANIFEST_PATH))) {
  console.error("Usage: npx electron scripts/test-ai-phase2-infer.mjs <manifest.json>");
  console.error(`Run Phase 1 first: npx electron scripts/test-ai-phase1-decode.mjs`);
  process.exit(1);
}
...
for (const m of manifest) {
  ...
  const px = new Uint8ClampedArray(buf.buffer.slice(buf.byteOffset + 12, buf.byteOffset + 12 + w * h * c));
  decoded.push({ filename: m.file, data: px, width: w, height: h, channels: c });
}
...
const { AutoTokenizer, CLIPTextModelWithProjection, CLIPVisionModelWithProjection, env } = transformers;
...
if (fs.existsSync(path.join(modelDir, "Xenova", "clip-vit-base-patch32", "onnx", "model_quantized.onnx"))) {
  env.localModelPath = modelDir; env.allowRemoteModels = false;
} else {
...
for (const img of decoded) {
  try {
    ...
    const pixelData = buildVisionTensor(img.data, img.width, img.height, img.channels);
...
console.log(`     483张预估: ${((avgMs * 483) / 1000 / 60).toFixed(1)}分钟 (纯推理)`);
...
try {
  ...
  for (const qv of queryVectors) {
    ...
    const top = results.slice(0, 3).map((r) => {
      ...
      return { id: r.photo_id, sim: Math.round((1 - r._distance) * 10000) / 10000, f: iv?.filename?.slice(0, 16) || "?" };
    });
    console.log(`  "${qv.zh}": ${ms}ms ${ms < 500 ? '✅' : '❌'} | Top:[${top.map((t) => `${t.f}(${t.sim.toFixed(3)})`).join(", ")}]`);
  }
