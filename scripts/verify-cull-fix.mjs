...
function simulateOld() {
  ...
  function createLoadPair() {
    ...
    return {
      ...
      call: () => {
        callCount++;
        callLog.push(
          `OLD loadPair#${v} called (deps: completedComparisons=${completedComparisons})`
        );
      },
    };
  }
  ...
  for (let i = 0; i < 10; i++) {
    ...
    if (
      i === 0 ||
      loadPair.deps.completedComparisons !== completedComparisons
    ) {
...
function simulateNew() {
  ...
  function createLoadPair() {
    ...
    return {
      ...
      call: () => {
        callCount++;
        callLog.push(
          `NEW loadPair#${v} called (deps: sessionId=${sessionId}, status=${sessionStatus})`
        );
      },
    };
  }
  ...
  for (let i = 0; i < 10; i++) {
    ...
    if (
      !initialLoadCalled ||
      loadPair.deps.sessionId !== sessionId ||
      loadPair.deps.sessionStatus !== sessionStatus
    ) {
...
console.log(
  `  旧代码: ${oldCalls} 次 loadPair 调用 (${(oldCalls / 10).toFixed(1)}x / 次操作)`
);
console.log(
  `  新代码: ${newCalls} 次 loadPair 调用 (${(newCalls / 10).toFixed(1)}x / 次操作)`
);
