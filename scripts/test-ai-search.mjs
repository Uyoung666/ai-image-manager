...
for (
  let i = 10;
  sampleFiles.length < SAMPLE && i < dirFiles.length;
  i += Math.max(1, Math.floor(dirFiles.length / SAMPLE))
) {
  sampleFiles.push(dirFiles[i]);
}
console.log(
  `  样本: ${sampleFiles.length} 张 (从 ${dirFiles.length} 有效图片中均匀采样)`
);
...
const decodeAvg =
  decodedImages.length > 0 ? totalDecodeMs / decodedImages.length : 0;
console.log(
  `  ✅ ${decodedImages.length} 张预解码完成, avg ${decodeAvg.toFixed(0)}ms/张`
);
...
console.log(
  `  PDR <100ms/CPU: ${(decodeAvg + avgMs) < 100 ? "✅" : decodeAvg + avgMs < 200 ? "⚠️ 接近" : "❌"}`
);
console.log(
  `  483张预估: ${(((decodeAvg + avgMs) * 483) / 1000 / 60).toFixed(1)}分钟`
);
...
try {
  ...
  const { Field, FixedSizeList, Float32, Float64, Int32, Schema } =
  ...
  for (const qv of queryVectors) {
    ...
    const top = results.slice(0, 3).map((r) => ({
      ...
      file:
        imageVectors
          .find((iv) => iv.id === r.photo_id)
          ?.filename?.slice(0, 20) || "?",
    }));
    const pdr = ms < 500 ? "✅" : "❌";
    console.log(
      `  "${qv.zh}": ${ms}ms ${pdr} | Top:[${top.map((t) => `${t.file}(sim=${t.sim.toFixed(3)})`).join(", ")}]`
    );
  }
  const avgSearch =
