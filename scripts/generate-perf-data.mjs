...
const COUNT = parseInt(arg("--count", "100000"), 10);
...
const ASPECT_RATIOS = [
  { ar: 3 / 2, weight: 0.40 }, // DSLR default
  { ar: 4 / 3, weight: 0.15 }, // m4/3 + phones
  { ar: 16 / 9, weight: 0.10 }, // widescreen phones
...
function generateMinimalJpeg(width, height) {
  ...
  const side = Math.max(2, Math.min(64, Math.floor(Math.min(width, height) / 10)));
...
function pickAspectRatio() {
  ...
  for (let i = 0; i < cumWeights.length; i++) {
    if (r <= cumWeights[i]) return ASPECT_RATIOS[i].ar;
  }
  return 3 / 2;
}
...
const cameraModels = [
  "Sony A7M4", "Sony A7M3", "Sony A7R5", "Canon EOS R5", "Canon EOS R6",
  "Nikon Z8", "Nikon Z6II", "Fujifilm X-T5", "Fujifilm X100VI", null,
];
const lensModels = [
  "FE 24-70mm F2.8 GM II", "FE 70-200mm F2.8 GM II", "FE 50mm F1.2 GM",
  "RF 24-70mm F2.8 L", "RF 50mm F1.2 L", "NIKKOR Z 24-70mm F2.8 S", null,
];
...
const insertAll = sqlite.transaction(() => {
  for (let i = 1; i <= COUNT; i++) {
    ...
    const dateTaken = Date.now() - Math.floor(Math.random() * 5 * 365 * 24 * 60 * 60 * 1000);
    ...
    insertPhoto.run(filePath.replace(/\\/g, "/"), filename, fileSize, fileDate, width, height);
    ...
    if (Math.random() > 0.3) {
      ...
      const aperture = [1.2, 1.4, 1.8, 2.8, 4, 5.6, 8][Math.floor(Math.random() * 7)];
      const iso = [100, 200, 400, 800, 1600, 3200, 6400][Math.floor(Math.random() * 7)];
    ...
    if (i % batchSize === 0) {
      ...
      console.log(`  ${i.toLocaleString()}/${COUNT.toLocaleString()} (${pct}%) — 本批 ${batchSize} 条 ${elapsed}s`);
      lastReport = Date.now();
    }
  }
});
...
console.log(`   数据库: ${DB_PATH} (${(fs.statSync(DB_PATH).size / 1024 / 1024).toFixed(1)} MB)`);
console.log(`   图片目录: ${IMG_DIR} (${fs.readdirSync(IMG_DIR).length.toLocaleString()} 文件)`);
...
function bench(name, fn, iterations = 10) {
  ...
  console.log(`  ${name.padEnd(40)} avg=${avg.toFixed(2)}ms  p50=${p50.toFixed(2)}ms  p95=${p95.toFixed(2)}ms  p99=${p99.toFixed(2)}ms`);
}

// Paginated list (simulates listPhotos with limit 100)
bench("listPhotos (LIMIT 100, OFFSET 0)", () => {
  sqlite.prepare("SELECT * FROM photos ORDER BY file_date DESC LIMIT 100").all();
});

bench("listPhotos (LIMIT 100, OFFSET 50000)", () => {
  sqlite.prepare("SELECT * FROM photos ORDER BY file_date DESC LIMIT 100 OFFSET 50000").all();
});
...
bench("search by filename LIKE pattern", () => {
  sqlite.prepare("SELECT * FROM photos WHERE filename LIKE '%IMG_05%' LIMIT 100").all();
});
