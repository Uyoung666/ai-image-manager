...
function countPhotos(db) {
  return db
    .prepare("SELECT count(*) AS c FROM photos WHERE deleted_at IS NULL")
    .get().c;
}
...
if (!fs.existsSync(DB_PATH)) {
  ...
  console.error(
    "  Generate test data first: node scripts/generate-perf-data.mjs --count 10000"
  );
  process.exit(1);
}
...
results.push(
  bench("list_first_page (OFFSET 0 LIMIT 100)", () => {
    db.prepare(`
...
results.push(
  bench("list_deep_page (OFFSET 5000 LIMIT 100)", () => {
    db.prepare(`
    SELECT id, filename, file_date, file_size, width, height,
           format, thumbnail_path, is_favorite
    FROM photos
...
results.push(
  bench("exif_date_range", () => {
    db.prepare(`
    SELECT e.photo_id FROM exif_data e
    WHERE e.date_taken >= ? AND e.date_taken <= ?
    LIMIT 100
  `).all(new Date("2024-01-01").getTime(), new Date("2024-12-31").getTime());
  })
);
...
results.push(
  bench("exif_compound (date+camera+iso)", () => {
    db.prepare(`
    SELECT e.photo_id FROM exif_data e
    WHERE e.date_taken >= ? AND e.date_taken <= ?
...
const buckets = [
  ...new Set([(targetHue + 35) % 36, targetHue, (targetHue + 1) % 36]),
];

results.push(
  bench("color_search_hue_bucket (#FF6B35)", () => {
    db.prepare(`
    SELECT id, closest_color_dist(255, 107, 53, dominant_colors) AS dist
    FROM photos
    WHERE deleted_at IS NULL AND dominant_colors IS NOT NULL
...
results.push(
  bench("getStats_time_aggregates", () => {
    const _hourQuery = db
      .prepare(`
    SELECT
...
results.push(
  bench("getExifCandidates (6 queries)", () => {
    db.prepare(
      "SELECT DISTINCT camera_model FROM exif_data WHERE camera_model IS NOT NULL AND camera_model != '' ORDER BY camera_model"
    ).all();
    db.prepare(
      "SELECT lens_model FROM exif_data WHERE lens_model IS NOT NULL AND lens_model != '' GROUP BY lens_model ORDER BY count(*) DESC"
    ).all();
    db.prepare(
      "SELECT DISTINCT CAST(ROUND(focal_length_num, 0) AS TEXT) FROM exif_data WHERE focal_length_num IS NOT NULL ORDER BY ROUND(focal_length_num, 0)"
    ).all();
    db.prepare(
      "SELECT DISTINCT ROUND(aperture, 1) FROM exif_data WHERE aperture IS NOT NULL ORDER BY ROUND(aperture, 1)"
    ).all();
    db.prepare(
      "SELECT DISTINCT iso FROM exif_data WHERE iso IS NOT NULL ORDER BY iso"
    ).all();
    db.prepare(
      "SELECT DISTINCT format FROM photos WHERE format IS NOT NULL AND format != '' ORDER BY format"
    ).all();
  })
);
...
results.push(
  bench("smart_album_composite", () => {
    // 模拟多规则求值：日期 + 相机 + 镜头
    const dateRangeIds = db
      .prepare(`
...
console.log(
  `║ ${"Scenario".padEnd(nameWidth)}${fmt("P50(ms)")}${fmt("P95(ms)")} ║`
);
console.log(
  "╟──────────────────────────────────────────────────────────────────────────────╢"
);
