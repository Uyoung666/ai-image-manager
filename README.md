...
**安装版（推荐）：** `AI Image Manager-1.3.2 Setup.exe` — 安装到系统默认位置，创建快捷方式，支持自动更新。

**便携版：** `AI Image Manager-win32-x64-1.3.2.zip` — 解压即用，不写注册表。
...
**三步引导向导** — 首次启动会自动进入引导流程：

1. **数据目录选择** — 设置照片数据的存储位置（缩略图/向量库/AI 模型），支持从已有目录迁移
2. **GPU 加速设置** — 一键检测 DirectML 兼容性，开启后人脸检测速度提升 6.6 倍
3. **完成** — 语言根据系统自动选择（中文/English），AI 模型已内置在安装包中无需下载

如需重新检测 GPU 或修改设置：**设置 → GPU 加速**。
