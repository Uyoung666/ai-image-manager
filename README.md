...
两种安装方式都通过应用内更新接收 GitHub 新版本。更新只识别已经正式发布、版本标签符合 SemVer、且不是草稿或预发布的 GitHub Release；普通代码推送和仍处于 Draft 状态的 Release 不会推送给客户端。Windows Release 必须同时包含 `RELEASES`、`*-full.nupkg` 和 `*Setup.exe`，发布工作流会自动校验这些文件。

如果曾使用自动更新项显示红叉的早期 MSI，请先用新版 MSI 覆盖安装一次；从这次安装开始，后续版本即可由应用自动检测、下载和更新。
