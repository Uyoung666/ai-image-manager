...
以下是当前版本目前已记录的两组实际观测值。表中 10,000 张照片数据来自本机一次真实测试，数值四舍五入到秒：

| 工作量 | 导入 | AI 嵌入 | 总耗时 |
| ---: | ---: | ---: | ---: |
| 1,000 张照片 | 30 秒 | 32 秒 | **62 秒** |
| 10,000 张 JPG 照片 | 约 340 秒（5 分 40 秒） | 约 746 秒（12 分 26 秒） | **约 1,086 秒（18 分 06 秒）** |

10,000 张测试使用 `D:\8806\10000` 中的 10,000 张 JPG（源文件总量约 2.27 GB），在独立数据目录中完成导入、缩略图/EXIF/哈希准备和 SigLIP 嵌入后删除了全部派生数据。原始测量值为导入 339.587 秒、AI 嵌入 746.478 秒、总计 1,086.065 秒。测试硬件、图片分辨率和系统负载未完整记录，因此这些数据仅作为当前版本的单次性能观测，不代表所有设备、图库或图片类型，也不用于外推其他规模的固定耗时。
