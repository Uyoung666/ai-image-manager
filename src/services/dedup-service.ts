...
import { BKTree, hammingDistance } from "./bk-tree";
...
export function checkNewPhotoDuplicates(
  ...
  // Check exact duplicates by content hash
  const sameSize = db
    .select({ id: photos.id, path: photos.path, contentHash: photos.contentHash })
    .from(photos)
