...
import { startWatching, stopWatching, stopScanning } from "@/services/indexer";
...
class ServiceRegistry {
  ...
  private async startServices(opts: {
    ...
    for (const name of order) {
      if (opts.skipStarted && started.has(name)) continue;

      const svc = this.services.get(name)!;

      if (opts.maxLevel !== undefined && svc.level > opts.maxLevel) continue;
      ...
      try {
        ...
        if (svc.level === ServiceLevel.Critical) {
          ...
          throw new Error(
            `Critical service "${name}" failed to start: ${msg}`
          );
        }
  ...
  async health(): Promise<HealthReport> {
    ...
    for (const [name, svc] of this.services) {
      try {
        ...
        if (h.status !== "ok") allHealthy = false;
      } catch (err: unknown) {
        results.push({
          name,
          level: svc.level,
...
registry.register({
  ...
  stop: async () => {
    // LanceDB closes automatically; no explicit teardown
  },
  health: async () => {
