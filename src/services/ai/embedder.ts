  ...
  return new Promise((resolve, reject) => {
    ...
    child.on("message", (msg: any) => {
      if (msg.type === "init-progress") {
        ...
        if (total > 0) {
          console.log(`[AI] CLIP download ${downloadPercent}% (${loaded}/${total})`);
        }
        return;
      }
      if (msg.type === "init-error" && !resolved) {
        appendAiWorkerLog(`[Batch Worker] init-error ${msg.error || "Worker init failed"}`);
        resolved = true;
      ...
      if (msg.type === "result" && !resolved) {
        appendAiWorkerLog(
          `[Batch Worker] result ok=${(msg.results as EmbedResult[]).filter((r) => r.vector && r.vector.length > 0).length} errors=${(msg.results as EmbedResult[]).filter((r) => r.error).map((r) => `${r.id}:${r.error}`).join(" | ").slice(0, 1000)}`
        );
        resolved = true;
        clearTimeout(timeout);
...
export async function embedAllPhotos(
  ...
  const unprocessed = db
    ...
    .from(photos)
    .where(
      sql`${photos.isAiProcessed} = 0 AND ${photos.deletedAt} IS NULL`
    )
    .all();
  ...
  if (total === 0 && totalPhotos === 0) {
    ...
    setCurrentProgress({
      ...
      currentFile: "",
      downloadPercent: undefined,
      error: "AI 嵌入失败：Worker 进程未能处理任何照片，请检查模型文件和依赖是否完整",
    });
  } else {
    setCurrentProgress({
      processed,
