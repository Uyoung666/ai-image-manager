...
export const listCloudConfigs = os.handler(() => {
  ...
  return db.select().from(cloudConfigs).orderBy(desc(cloudConfigs.createdAt)).all();
});
...
export const createCloudConfig = os
  ...
  .handler(async ({ input }) => {
    ...
    const result = db
      ...
      .get();
    return db.select().from(cloudConfigs).where(eq(cloudConfigs.id, result!.insertedId)).get();
  });
