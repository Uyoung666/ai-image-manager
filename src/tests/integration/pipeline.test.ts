...
describe("Pipeline Integration Test (500 images)", () => {
  ...
  describe("Step 1: 文件发现与格式检测", () => {
    it("应正确发现测试目录中的500张图片", () => {
      const files = fs
        ...
        .filter((f) => /\.(jpg|jpeg|png|webp|avif|tiff?|heic|heif|gif|bmp)$/i.test(f));
  ...
  describe("Step 5: AI 模型可用性检查", () => {
    it("应能找到CLIP模型文件", () => {
      ...
      const candidates = [
        path.join(TEST_DATA_DIR, "models", "Xenova", "clip-vit-base-patch32", "onnx", "model_quantized.onnx"),
        path.join(process.cwd(), "models", "Xenova", "clip-vit-base-patch32", "onnx", "model_quantized.onnx"),
        path.join(os.homedir(), "AppData", "Roaming", "ai-image-manager", "models", "Xenova", "clip-vit-base-patch32", "onnx", "model_quantized.onnx"),
      ];
  ...
  describe("汇总报告", () => {
    it("应输出完整性能报告", () => {
      ...
      console.log(`有缩略图: ${allPhotos.filter((p: any) => p.thumbnailPath).length}`);
      console.log(`有 pHash: ${allPhotos.filter((p: any) => p.phash).length}`);
      console.log(`数据库文件: ${path.join(TEST_DATA_DIR, "data", "ai-image-manager.db")}`);
