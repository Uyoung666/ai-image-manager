...
describe("路径安全验证", () => {
  describe("isSafePath", () => {
    ...
    it("应该防止路径遍历攻击 (..)", () => {
      const allowedRoots = ["C:\\Users\\test\\Documents"];
      const targetPath =
        "C:\\Users\\test\\Documents\\..\\..\\Windows\\System32";
    ...
    it("应该处理多个白名单根目录", () => {
      ...
      expect(
        isSafePath("C:\\Users\\test\\Documents\\file.txt", allowedRoots)
      ).toBe(true);
      expect(
        isSafePath("C:\\Users\\test\\Pictures\\photo.jpg", allowedRoots)
      ).toBe(true);
      expect(isSafePath("D:\\Photos\\vacation.jpg", allowedRoots)).toBe(true);
      expect(
        isSafePath("C:\\Users\\test\\Downloads\\file.txt", allowedRoots)
      ).toBe(false);
    });
