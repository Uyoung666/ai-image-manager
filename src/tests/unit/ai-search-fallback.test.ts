...
function translateChineseQuery(query: string): string {
  ...
  if (englishOnly.length === 0) return query.trim();
...
function bruteForceScan(
  ...
  return storedVectors
    ...
    .map((s) => {
      let dot = 0;
      for (let i = 0; i < queryVector.length; i++) dot += queryVector[i] * s.vector[i];
...
function adaptiveRefineFactor(rowCount: number): number {
  return Math.min(10, Math.max(3, Math.ceil(100 / Math.sqrt(Math.max(rowCount, 1)))));
}
...
describe("filterByThreshold", () => {
  const mockResults = [
    ...
    { photo_id: 2, _distance: 0.40 },
    { photo_id: 3, _distance: 0.52 },
    { photo_id: 4, _distance: 0.60 },
  ...
  it("所有结果都差时返回 top 5", () => {
    const allPoor = [
      { photo_id: 10, _distance: 0.70 },
      { photo_id: 20, _distance: 0.75 },
      { photo_id: 30, _distance: 0.80 },
      { photo_id: 40, _distance: 0.85 },
      { photo_id: 50, _distance: 0.90 },
...
describe("adaptiveRefineFactor", () => {
  it("小数据集(<256) 使用更大的 refineFactor", () => {
    ...
    const large = adaptiveRefineFactor(10000);
