...
export function SignatureOverlay({ active, onDone }: SignatureOverlayProps) {
  const [phase, setPhase] = useState<"idle" | "drawing" | "done" | "fading">(
    "idle"
  );
  ...
  const colors = isDark
    ? {
        strokeGrad: {
          id: "ds",
          stops: ["#E0E7FF", "#FFFFFF", "#C7D2FE", "#A5B4FC"],
        },
    ...
    : {
        strokeGrad: {
          id: "ls",
          stops: ["#1E3A5F", "#2D4A7A", "#1A365D", "#2B6CB0"],
        },
  ...
  const makeGradient = (
    g: typeof colors.strokeGrad,
    x2 = "100%",
    y2 = "0%"
  ) => (
    <linearGradient id={g.id} x1="0%" x2={x2} y1="0%" y2={y2}>
      {g.stops.map((stop, i) => (
        <stop
          key={stop}
    ...
    <div
      ...
      style={{
        ...
        transition: reduceMotion
          ? "none"
          : "opacity 800ms ease-out, background-color 400ms ease",
      }}
    >
      <svg
      ...
      >
        <defs>
          ...
          {colors.hasGlow && (
            <filter height="180%" id="sigGlow" width="180%" x="-40%" y="-40%">
              <feGaussianBlur in="SourceGraphic" result="b1" stdDeviation="2" />
              <feGaussianBlur in="SourceGraphic" result="b2" stdDeviation="5" />
              <feMerge>
                <feMergeNode in="b2" />
                <feMergeNode in="b1" />
          ...
          {!colors.hasGlow && (
            <filter height="140%" id="sigShadow" width="140%" x="-20%" y="-20%">
              <feDropShadow
                dx="0.5"
                dy="1"
                floodColor="#1E3A5F"
        ...
        <circle
          ...
          r="3.5"
          style={{
            transition: reduceMotion ? "none" : "opacity 0.3s ease 1.8s",
          }}
        />
      </svg>
