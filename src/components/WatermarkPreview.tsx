...
export function calcWmPosition(
  ...
  let x: number;
  if (h === "left") x = mp;
  else if (h === "right") x = imgW - wmW - mp;
  else x = Math.round((imgW - wmW) / 2);

  let y: number;
  if (v === "top") y = mp;
  else if (v === "bottom") y = imgH - wmH - mp;
  else y = Math.round((imgH - wmH) / 2);

  return { x, y };
}

export function WatermarkPreview({ wm, samplePhotoPath, onSettingsChange }: Props) {
  ...
  useEffect(() => {
    if (!samplePhotoPath) { setSampleImg(null); return; }
    const img = new Image();
  ...
  useEffect(() => {
    if (!wm.imagePath) { setWmImg(null); return; }
    const img = new Image();
  ...
  const draw = useCallback(() => {
    ...
    if (sampleImg && !loadError) {
      ...
      let sx = 0, sy = 0, sw = sampleImg.width, sh = sampleImg.height;
    ...
    let v: "top" | "center" | "bottom";
    const a = wm.anchor;
    if (a === "topLeft") { h = "left"; v = "top"; }
    else if (a === "topCenter") { h = "center"; v = "top"; }
    else if (a === "topRight") { h = "right"; v = "top"; }
    else if (a === "centerLeft") { h = "left"; v = "center"; }
    else if (a === "center") { h = "center"; v = "center"; }
    else if (a === "centerRight") { h = "right"; v = "center"; }
    else if (a === "bottomLeft") { h = "left"; v = "bottom"; }
    else if (a === "bottomCenter") { h = "center"; v = "bottom"; }
    else { h = "right"; v = "bottom"; }

    const ax = h === "left" ? mp : h === "right" ? cw - mp : Math.round(cw / 2);
    const ay = v === "top" ? mp : v === "bottom" ? ch - mp : Math.round(ch / 2);

    let wmW = 0, wmH = 0, wmX = 0, wmY = 0;

    if (wmImg) {
      const maxDim = Math.round(Math.min(cw, ch) * (wm.imageScale / 100));
      const ratio = wmImg.width / wmImg.height;
      if (ratio > 1) { wmW = maxDim; wmH = maxDim / ratio; }
      else { wmH = maxDim; wmW = maxDim * ratio; }
    ...
    if (wmImg) {
      ...
      ctx.strokeStyle = dragging ? "rgba(94,106,210,0.8)" : "rgba(255,255,255,0.3)";
      ctx.lineWidth = 1;
      ctx.strokeRect(wmX, wmY, wmW, wmH);
    }
  ...
  function dragToAnchor(clientX: number, clientY: number) {
    ...
    const approxW = wmImg ? Math.round(Math.min(cw, ch) * (wm.imageScale / 100)) : cw * 0.12;
    const approxH = approxW;

    const candidates = ANCHORS.map((a) => {
      const pos = calcWmPosition(a.anchor, 5, cw, ch, approxW, approxH);
      const cx = pos.x + approxW / 2, cy = pos.y + approxH / 2;
    ...
    const marginPct = Math.round(Math.max(2, Math.min(15, (distPx / Math.min(cw, ch)) * 100)));

    onSettingsChange({ anchor: nearestAnchor, margin: marginPct });
  }

  function handleMouseDown(e: React.MouseEvent) { setDragging(true); dragToAnchor(e.clientX, e.clientY); }
  function handleMouseMove(e: React.MouseEvent) { if (!dragging) return; dragToAnchor(e.clientX, e.clientY); }
  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      <div className="flex items-center gap-3">
        ...
        <span className="text-[10px] text-muted-foreground/50">{t("orDragPreview")}</span>
      </div>
    </div>
  );
}
